 ...
 * LOA Framework - CLI
 *
 * Command-line interface for creating and running LOA projects.
 */
...
const PACKAGE_NAME = '@srvinix/loa';
const VERSION = '1.0.0';
...
const commands = {
  create: {
    description: 'Create a new LOA project',
    usage: 'loa create <project-name>',
    run: runCreate,
  },
  dev: {
    description: 'Start a built project entry file',
    usage: 'loa dev [entry-file]',
    run: runDev,
  },
  start: {
    description: 'Start a production server entry file',
    usage: 'loa start [entry-file]',
    run: runStart,
  },
  build: {
    description: 'Show build guidance for LOA projects',
    usage: 'loa build',
    run: runBuild,
  },
  version: {
    ...
    usage: 'loa version',
    run: () => console.log(`LOA v${VERSION}`),
  },
  help: {
    description: 'Show help',
    usage: 'loa help [command]',
    run: runHelp,
  },
};
...
async function runCreate(name) {
  ...
  console.log(`Creating LOA project: ${projectName}`);
...
function runBuild() {
  console.log('Use npm run build inside your LOA project.');
}
...
function validateProjectName(name) {
  if (!name) {
    throw new Error('Please provide a project name: loa create <project-name>');
  }
